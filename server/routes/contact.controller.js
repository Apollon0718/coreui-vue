...
exports.getContactById = async function (req, res, next) {
  ...
  try {
    let ContactByIdData = await ContactService.getContactById(id);
    //console.log(ContactByIdData)
    return res.status(200).json({ status: 200, data: ContactByIdData, message: "Successfully Edit page loaded" })
  } catch (e) {
    return res.status(400).json({ status: 400, message: e.message });
  }
};
